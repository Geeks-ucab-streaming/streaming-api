...
export class GetArtistProfilesApplicationService implements IApplicationService<GetArtistProfilesApplicationServiceDto, Artist>{
    ...
    async execute(dto?: GetArtistProfilesApplicationServiceDto): Promise<Result<Artist>>{
        const artist = await this.artistRepository.findOneByIdOrFail(ArtistID.create(dto.id));
